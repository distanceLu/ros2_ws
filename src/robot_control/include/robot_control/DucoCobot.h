...
struct MoveJogTaskParams
{
    ...
    double step_jointValue; // 关节和空间RX/RY/RZ步进弧度, 单位: rad
    double step_cartvalue;  // 空间X/Y/Z步进距离, 单位: m
};
...
struct TrajTimeParams
{
    ...
    int32_t type;                 // 运动类型 0:movej, 1:movej2, 2:movej_pose, 3:movej_pose2, 4:movel, 5:movec, 6:movetcp
    double vel;                   // 速度
    double acc;                   // 加速度
    bool circle;                  // 是否整圆
...
struct PathOffsetResult
{
    ...
    int32_t status_code;  // 任务状态信息, 参考TaskState
};
...
public:
	 ...
	 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
     */
    int32_t movec(const std::vector<double> & p1, const std::vector<double> & p2, double v, double a, double r, int mode, const std::vector<double> & q_near, const std::string& tool, const std::string& wobj, bool block, const OP &op = _op, bool def_acc = false, double arc_rad = 0) ;
    /**
     ...
     * @param v 关节速度, 范围[0.01*PI/180, 1.25*PI], 单位: m/s
     ...
     * @param _return
     */
    void get_flange_pose_cmd(std::vector<double> & _return);
    /**
     * @brief g法兰笛卡尔指令速度, 参考base坐标系
     * @param _return
     */
    void get_flange_speed_cmd(std::vector<double> & _return);
    /**
     * @brief 法兰笛卡尔指令加速度, 参考base坐标系
     * @param _return
     */
    void get_flange_acceleration_cmd(std::vector<double> & _return) ;
    /**
     ...
     */
    void get_tcp_pose_command(std::vector<double> & _return);
    /**
     * @brief 当前生效TCP笛卡尔指令速度, 参考base坐标系
     * @param _return 当前生效TCP笛卡尔指令速度
     */
    void get_tcp_speed_command(std::vector<double> & _return);
    /**
     * @brief 当前生效TCP笛卡尔指令加速度, 参考base坐标系
     * @param _return 当前生效TCP笛卡尔指令加速度
     */
    void get_tcp_acceleration_command(std::vector<double> & _return);
    /**
     ...
     */
    void get_tcp_pose_command_coord(std::vector<double> & _return, const std::string& tool, const std::string& wobj);
    /**
     ...
     * @param wobj 工件坐标系名称, 为空字符串默认为当前使用的坐标系
     */
    void get_tcp_speed_command_coord(std::vector<double> & _return, const std::string& tool, const std::string& wobj);
    /**
     ...
     * @param wobj 工件坐标系名称, 为空字符串默认为当前使用的坐标系
     */
    void get_tcp_acceleration_command_coord(std::vector<double> & _return, const std::string& tool, const std::string& wobj);
    /**
     ...
     * @param wobj 工件坐标系名称, 为空字符串默认为当前使用的坐标系
     */
    void get_tcp_speed_coord(std::vector<double> & _return, const std::string& tool, const std::string& wobj);
    /**
     ...
     * @param wobj 工件坐标系名称, 为空字符串默认为当前使用的坐标系
     */
    void get_tcp_acceleration_coord(std::vector<double> & _return, const std::string& tool, const std::string& wobj);
    /**
    ...
    /**
     * @brief 控制单个或多个外部轴移动
     * @param scheme_name 目标外部轴方案名称列表
     * @param epose 目标外部轴方案所对应自由度位置(三维)列表,
	 *              记录位置自由度及单位根据外部轴方案所设置自由度及外部轴方案类型改变, 单位: rad, m
     * @param v     外部轴最大规划速度列表, 根据对应外部轴方案类型改变, 单位: rad/s, m/s
     * @param block 指令是否阻塞型指令, 如果为false表示非阻塞指令, 指令会立即返回
     * @param op    详见上方Op特殊类型说明(距离触发无效),可缺省参数
     * @return 当配置为阻塞执行, 返回值代表当前任务结束时的状态, 若无融合为Finished, 若有融合为Interrupt.
	 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
     */
    int32_t move_eaxis(std::vector<std::string> & scheme_name, const std::vector<std::vector<double> > & epose, const std::vector<double> & eaxis_v, bool block, const OP& op = _op);
    /**
     * @brief 控制单个或多个外部轴和机器人联动, 且机器人执行关节运动
     ...
     * @param epose 目标外部轴方案所对应自由度位置(三维)列表,
	 *              记录位置自由度及单位根据外部轴方案所设置自由度及外部轴方案类型改变, 单位: rad, m
     * @param eaxis_v 外部轴最大规划速度列表, 根据对应外部轴方案类型改变, 单位: rad/s, m/s
     * @param block   指令是否阻塞型指令, 如果为false表示非阻塞指令, 指令会立即返回
     * @param op      详见上方Op特殊类型说明(距离触发无效), 可缺省参数
     * @param def_acc 是否使用系统默认加速度, false表示使用自定义的加速度值, true表示使用系统自动规划的加速度值, 可缺省, 默认为false
     * @return 当配置为阻塞执行, 返回值代表当前任务结束时的状态, 若无融合为Finished, 若有融合为Interrupt.
	 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
     */
    int32_t movej2_eaxis(const std::vector<double> & joints_list, double v, double a, double rad, std::vector<std::string> & scheme_name, const std::vector<std::vector<double> > & epose, const std::vector<double> & eaxis_v, bool block, const OP& op = _op, bool def_acc = false);
    /**
     * @brief 控制单个或多个外部轴和机器人联动, 且机器人从当前状态按照关节运动的方式移动到末端目标位置
     ...
     * @param wobj  设置使用的工件坐标系的名称, 为空时默认为当前使用的工件坐标系
     * @param scheme_name 目标外部轴方案名称列表
     * @param epose 目标外部轴方案所对应自由度位置(三维)列表, 记录位置自由度及单位根据外部轴方案所设置自由度及外部轴方案类型改变, 单位: rad, m
     * @param eaxis_v 外部轴最大规划速度列表, 根据对应外部轴方案类型改变, 单位: rad/s, m/s
     * @param block 指令是否阻塞型指令, 如果为false表示非阻塞指令, 指令会立即返回
     * @param op 详见上方Op特殊类型说明(距离触发无效),可缺省参数
     * @param def_acc 是否使用系统默认加速度, false表示使用自定义的加速度值, true表示使用系统自动规划的加速度值, 可缺省, 默认为false
	 ...
	 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
     */
    int32_t movej2_pose_eaxis(const std::vector<double> & p, double v, double a, double rad, const std::vector<double> & qnear, const std::string& tool, const std::string& wobj, std::vector<std::string> & scheme_name, const std::vector<std::vector<double> > & epose, const std::vector<double> & eaxis_v, bool block, const OP& op = _op, bool def_acc = false);
    /**
     * @brief 控制单个或多个外部轴和机器人联动, 且机器人从当前状态按照直线路径移动到目标状态
     * @param p 对应末端的位姿, 位置单位: m, 姿态以Rx、Ry、Rz表示, 范围[-2*PI, 2*PI], 单位: rad
     * @param v 末端速度, 范围[0.00001, 5], 单位: m/s
     * @param a 末端加速度, 范围[0.00001, ∞), 单位: m/s^2
     * @param rad 融合半径, 单位: m
	 ...
	 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
     */
    int32_t movel_eaxis(const std::vector<double> & p, double v, double a, double rad, const std::vector<double> & qnear, const std::string& tool, const std::string& wobj, std::vector<std::string> & scheme_name, const std::vector<std::vector<double> > & epose, const std::vector<double> & eaxis_v, bool block, const OP& op = _op, bool def_acc = false);
    /**
     * @brief 控制单个或多个外部轴和机器人联动, 且机器人做圆弧运动, 起始点为当前位姿点, 途径p1点, 终点为p2点
     * @param p1 圆弧运动中间点位姿
     * @param p2 圆弧运动结束点位姿
     * @param v 末端速度, 范围[0.00001, 5], 单位: m/s
     * @param a 末端加速度, 范围[0.00001, ∞), 单位: m/s^2
	 ...
	 *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
     */
    int32_t movec_eaxis(const std::vector<double> & p1, const std::vector<double> & p2, double v, double a, double rad, const std::vector<double> & qnear,
                        const std::string& tool, const std::string& wobj, std::vector<std::string> & scheme_name, const std::vector<std::vector<double> > & epose, const std::vector<double> & eaxis_v,
                        bool block, const OP& op = _op, bool def_acc = false, int32_t mode = 0, double arc_rad = 0);
    /**
     * @brief 控制单个或多个外部轴和机器人联动, 且机器人做圆周运动, 起始点为当前位姿点, 途径p1点和p2点
     * @param p1 圆周运动经过点
     * @param p2 圆周运动经过点
     * @param v 末端速度, 范围[0.00001, 5], 单位: m/s
     * @param a 末端加速度, 范围[0.00001, ∞), 单位: m/s^2
     ...
     *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
     */
    int32_t move_circle_eaxis(const std::vector<double> & p1, const std::vector<double> & p2, double v, double a, double rad, int32_t mode, const std::vector<double> & qnear, const std::string& tool, const std::string& wobj, std::vector<std::string> & scheme_name, const std::vector<std::vector<double> > & epose, const std::vector<double> & eaxis_v, bool block, const OP& op = _op, bool def_acc = false);
    /**
     * @brief 判断机器人是否能运动到队列里的目标位姿
     ...
     * @return 可达性检查的结果
     */
    ReachabilityParams reach_check( const std::vector<double> &base, const std::vector<double> &tool, const std::vector<double> &wobj, const std::vector<double> &ref_pos, const std::vector<std::vector<double> > &check_points);
    /**
     * @brief 控制外部轴和机器人执行点动
    ...
    */
    ...
    /**
     * @brief 获取机器人pose_list类型全局变量的值
     * @param value pose_list类型全局变量的值
     * @param name 全局变量的名称. 如果变量在域里, 名称格式为："域名.变量名"
     */
     ...
     * @param param 参考TrajTimeParams
     * @return  小于0: 参数不匹配, 等于0: 点位异常, 大于0: 用时
     */
    int64_t cal_traj_time(const TrajTimeParams& param);
    /**
     * @brief 获取主板硬件时钟, 精确到ms
     * @return 主板硬件时钟, 单位: ms
     */
    int64_t get_hardware_clock();
    /**
     * @brief 获取工具坐标系值
     * @param tool 工具坐标系值(长度:10)
     * @param name 为空: 返回当前坐标系的值, 不为空: 返回指定工具坐标系的值
     */
    void get_tool_data(std::vector<double> & tool, const std::string& name);
    /**
     * @brief 获取当前设置工具的负载质量及质心位置
     * @param _return 质量单位: kg,质心位置单位: m,[mass,x_cog,y_cog,z_cog]
     * @param name 为空: 返回当前的值, 不为空: 返回指定的值
     */
    void get_tool_load(std::vector<double> & load, const std::string& name);
    /**
     * @brief 获取当前设置的工件坐标系的值
     * @param wobj [x, y, z, rx, ry, rz]工件坐标系相对于基坐标系的位移, 单位: m, rad
     * @param name 为空: 返回当前的值, 不为空: 返回指定的值
     */
    void get_wobj(std::vector<double> & wobj, const std::string& name);
    /**
     * @brief 获取机器人安装位姿
     * @param base 安装位姿
     */
    void get_base(std::vector<double> & base);
    /**
     * @brief 获取Home位置
     * @param pose Home位置关节角
     */
    void get_home_pose(std::vector<double> & pose);
    /**
     ...
     * @return 任务结束时的状态
     */
    int32_t set_tool_data_workcell(const std::string& name, const std::vector<double> & tool, const std::vector<double> & payload, const std::vector<double> & inertia_tensor) ;
    /**
     ...
     * @return 任务结束时的状态
     */
    int32_t set_wobj_workcell(const std::string& name, const std::vector<double> & wobj);
    /**
     * @brief 控制单个或多个外部轴和机器人联动，且机器人做原地摆运动
     * @param tool 工具坐标系名称, 为空字符串默认为当前使用的坐标系
     * @param wobj 设置使用的工件坐标系的名称, 为空时默认为当前使用的工件坐标系
     * @param axis 复合运动参考轴向: 0为参考平面轴向1, 1为参考平面轴向2. 例如XOY 0代表X轴 1代表Y轴
     * @param time  延迟时间参数 单位ms 当且仅当eaxis_scheme_name为空时生效
     * @param scheme_name 外部轴方案名称列表
     * @param epose   目标外部轴方案所对应自由度位置(三维)列表,
     *                记录位置自由度及单位根据外部轴方案所设置自由度及外部轴方案类型改变, 单位: rad, m
     * @param eaxis_v 外部轴最大规划速度列表, 根据对应外部轴方案类型改变, 单位: rad/s, m/s
     * @param block   指令是否阻塞型指令, 如果为false表示非阻塞指令, 指令会立即返回
     ...
     *         当配置为非阻塞执行, 返回值代表当前任务的id, 用户可以调用get_noneblock_taskstate(id)函数查询当前任务的执行状态
     */
    int32_t move_fixed(const std::string& tool, const std::string& wobj, const int32_t axis, const int32_t time, std::vector<std::string> & scheme_name, const std::vector<std::vector<double> > & epose, const std::vector<double> & eaxis_v, const bool block, const OP& op = _op, const bool def_acc = false);
    /**
     ...
     * @return 阻塞执行代表任务结束时状态, 非阻塞执行代表任务的ID
     */
    int32_t track_enqueue_op_vel(const std::vector<PointOP> & track, const bool block);
    /**
     * @brief 执行时, 机器人的工具末端tool将顺序到达轨迹池中的点位值直到轨迹池中无新的点位.
     *        执行过程中, 工具末端tool将以speed与acc在工件坐标系wobj下规划运动.
     *        注:如果已经开始执行停止规划, 将不再重新获取轨迹池中的数据, 直到完成停止.
     ...
     * @return 阻塞执行代表任务结束时状态, 非阻塞执行代表任务的ID
     */
    int32_t track_cart_vel_motion(const double acc, const std::string& tool, const std::string& wobj, const bool block);
    /**
     ...
     * @param path_offset 路径在路径坐标系起点处的偏移, 两个元素
     * @param path_retract 路径在路径坐标系终点处的缩进, 两个元素
     * @param tool_rotation 工具自身参考tool的旋转量, 两个元素
     * @param path_rotation 工具参考路径坐标系起点处路径切线方向的旋转量
     * @param weld_rotation 相对于焊道坐标系的RY、RZ, 可缺省, 两个元素
     * @param tool_z_offset 工具z偏移量, 可缺省
     * @param path_type     路径类型序列, 可缺省
     * @param plane_normals 法向量, 可缺省, 三个元素
     * @param average_height 根据法向量高度平均, 可缺省
     * @return 偏移后的路径点序列
     */
    PathOffsetResult path_offset_cal(const int32_t path_type, const std::vector<std::vector<double> > & path_point, const std::string& tool,
                                     const std::string& wobj, const std::vector<double> & path_offset, const std::vector<double> & path_retract,
                                     const std::vector<double> & tool_rotation, const double path_rotation, const std::vector<double> & weld_rotation = std::vector<double>(),
                                     const int32_t tool_z_offset = 0, const std::vector<int32_t> & path_type_list = std::vector<int32_t>(), const std::vector<double> & plane_normals = std::vector<double>(), bool average_height = false);
    /**
     * @brief 设置机械臂的安装方式
     * @param installation 安装方式。 两个元素{ry, rz},ry:机器人绕基座倾斜弧度, rz:机器人绕基座旋转弧度
     * @return  返回值代表当前任务结束时的状态
     */
    int32_t set_installation(const std::vector<double> & installation);
    /**
     * @brief 获取机械臂的安装方式
     * @return  当前安装方式。 两个元素{ry, rz},ry:机器人绕基座倾斜弧度, rz:机器人绕基座旋转弧度
     */
    std::vector<double> get_installation();
    /**
     * @brief 开启激光跟踪功能
     * @return 非阻塞执行代表任务的ID
     */
    int32_t enable_laser_track();
    /**
     * @brief 设置激光跟踪功能相关参数
     ...
     * @return 任务结束时的状态
     */
    int32_t set_laser_track_params(const int32_t plane, const int32_t freq, const int32_t lead_len, const std::vector<double> & active,
                                   const std::vector<double> & kp, const std::vector<double> & kd, const std::vector<double> & offset,
                                   const std::vector<double> & offset_vel, const std::vector<double> & filter_coe, const bool path_length_correction, const bool track_type);
    /**
     * @brief 关闭激光跟踪功能
     * @return 任务结束时的状态
     */
    int32_t disable_laser_track();
    /**
     * @brief 清空记录的偏移数据
     * @return 任务结束时的状态
     */
    int32_t reset_laser_track();
    /**
     * @brief 添加基于世界坐标系偏移的点位
     * @param pos 点位
     * @return 任务结束时的状态
     */
    int32_t write_laser_search_pos(const std::vector<double> & pos);
