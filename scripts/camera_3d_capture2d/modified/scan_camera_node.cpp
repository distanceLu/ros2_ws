...
public:
  ...
  : Node("scan_camera_node")
  {
    ...
    declare_parameter<bool>("use_projector_capturing_2d_image", false);
...
private:
  ...
  void load_capture_config()
  {
    ...
    capture_2d_options_ = RVC::X2::CaptureOptions();
    capture_2d_options_.exposure_time_2d = get_parameter("exposure_time_2d").as_int();
    capture_2d_options_.gain_2d = get_parameter("gain_2d").as_double();
    capture_2d_options_.gamma_2d = get_parameter("gamma_2d").as_double();
    capture_2d_options_.use_projector_capturing_2d_image =
      get_parameter("use_projector_capturing_2d_image").as_bool();
  }
  ...
  void initialize_ros_interfaces()
  {
    ...
    capture_2d_srv_ = create_service<std_srvs::srv::Trigger>(
      "capture_2d",
      [this](
        const std::shared_ptr<std_srvs::srv::Trigger::Request>,
        std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
        ...
        stop_fixed_line_scan_locked();
        std::string detail;
        response->success = capture_2d_once_locked(nullptr, &detail);
        response->message = response->success ?
          "2d capture completed without projector" : detail;
      });
    ...
    std::string * detail = nullptr)
  {
    ...
    stop_fixed_line_scan_locked();
    if (!camera_.Capture2D(image_camera_id_, capture_2d_options_)) {
      if (detail != nullptr) {
        *detail = std::string("RVC Capture2D failed: ") + RVC::GetLastErrorMessage();
      }
      RCLCPP_WARN(
        get_logger(), "RVC Capture2D failed: %s", RVC::GetLastErrorMessage());
      return false;
    }

    const auto image = camera_.GetImage(image_camera_id_);
    const auto cv_image = rvc_image_to_mat(image);
    if (cv_image.empty()) {
      if (detail != nullptr) {
        *detail = "Capture2D returned empty image";
      }
      return false;
    }
    ...
    const auto encoding = cv_image.channels() == 1 ? "mono8" : "bgr8";
    auto image_msg = mat_to_image_msg(cv_image, stamp, frame_id, encoding);
    ...
    if (detail != nullptr && detail->empty()) {
      *detail = "2d capture completed without projector";
    }
    return true;
  }
  ...
  RVC::X2::CaptureOptions capture_2d_options_;
  double y_min_{-0.01};
  double y_max_{0.01};
  ...
  rclcpp::Service<std_srvs::srv::Trigger>::SharedPtr capture_2d_srv_;
