...
只依赖本脚本完成三种相机的统一拍照：
  ...
  3. 熔池相机：pool_camera_node，默认话题 /pool_camera/image_raw
...
class UnifiedCameraCaptureNode(Node):
    ...
    def _ros2_command(self, args: list[str], timeout_s: float = 3.0) -> subprocess.CompletedProcess:
        setup = "source /home/shugen/ros2_ws/install/setup.bash"
        command = " ".join(args)
    ...
    def _start_driver_if_needed(self, state: CameraState) -> tuple[bool, str]:
        ...
        command = (
            f"source /home/shugen/ros2_ws/install/setup.bash && "
            f"ros2 launch {spec.launch_package} {spec.launch_file}"
        )
