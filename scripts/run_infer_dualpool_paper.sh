#!/usr/bin/env bash
# 一键启动 ACT 三相机推理（pool+pool1+paper_aruco，无 scan_2d）
#
# 用法:
#   ./run_infer_dualpool_paper.sh              # 正式执行（机械臂会动）
#   ./run_infer_dualpool_paper.sh dry-run      # dry-run（机械臂不动，只验证链路）
#   ./run_infer_dualpool_paper.sh attach       # 重新进入已存在的会话
#   ./run_infer_dualpool_paper.sh kill         # 停止全部
#
# 环境变量（可选）:
#   BRUSH_CKPT_DIR           三目权重目录，默认 2026_07_21_task_discrete_b1
#   BRUSH_TASK_NAME          constants.py 中的任务名
#   INFER_CAMERA_NAMES       bridge 相机名（空格分隔），默认 pool pool1 paper_aruco（无 scan）
...
#   PAPER_CAMERA_DEVICE      纸面 USB 相机，默认 by-id Capture 节点（当前机 → /dev/video1）
...
#   INFER_RECORD_ROOT        录制根目录，默认 /data/yanjie/data_collect
...
SESSION="${SESSION:-brush_infer_dualpool_paper}"
...
BRUSH_CKPT_DIR="${BRUSH_CKPT_DIR:-/media/shugen/LcxDisk/checkpoint/brush_policy_3cam_2026_08_12_dualpool_paper_cmd_delta_discrete_b1_perdim}"
BRUSH_TASK_NAME="${BRUSH_TASK_NAME:-brush_tool_pose_2026_08_12_dualpool_paper_cmd_delta}"
INFER_CAMERA_NAMES="${INFER_CAMERA_NAMES:-pool pool1 paper_aruco}"
...
INFER_RECORD_CSV="${INFER_RECORD_CSV:-/tmp/infer_dualpool_paper.csv}"
...
INFER_RECORD_ROOT="${INFER_RECORD_ROOT:-/data/yanjie/data_collect}"
...
cmd_start() {
  ...
  local infer_once_cmd="bash scripts/run_infer_brush_robot.sh --io_backend zmq --observation_zmq tcp://127.0.0.1:5554 --target_zmq tcp://127.0.0.1:5555 --max_timesteps ${INFER_MAX_TIMESTEPS} --device ${INFER_DEVICE} --observation_timeout_sec ${INFER_OBSERVATION_TIMEOUT_SEC} --debug_chunk --target_delta_gain ${INFER_TARGET_DELTA_GAIN} --max_amplified_step_m ${INFER_MAX_AMPLIFIED_STEP} --sleep_sec ${INFER_SLEEP_SEC} --task_id ${INFER_TASK_ID} --discrete_decode ${INFER_DISCRETE_DECODE} --discrete_temperature ${INFER_DISCRETE_TEMPERATURE} --record_targets_csv ${INFER_RECORD_CSV} ${qpos_flag}"
  local infer_hint=""
  local rl_exports=""
  if [[ -n "${INFER_ONCE_CMD:-}" ]]; then
    infer_once_cmd="${INFER_ONCE_CMD}"
    infer_hint="echo '[hint] 聚焦本窗口：↑/w = +Z，↓/s = -Z，q 急停'; "
    echo "INFER_ONCE_CMD 覆盖 infer 窗口命令"
    rl_exports="export RL_OUTPUT_DIR='${RL_OUTPUT_DIR:-}' RL_TOTAL_EPISODES='${RL_TOTAL_EPISODES:-}' RL_TOTAL_TIMESTEPS='${RL_TOTAL_TIMESTEPS:-}' RL_REPLAY_BUFFER_SIZE='${RL_REPLAY_BUFFER_SIZE:-}' RL_REPLAY_WARMUP_EPISODES='${RL_REPLAY_WARMUP_EPISODES:-}' RL_TRAIN_BATCH_SIZE='${RL_TRAIN_BATCH_SIZE:-}' RL_TRAIN_STEPS_PER_CYCLE='${RL_TRAIN_STEPS_PER_CYCLE:-}' RL_COLLECT_EPISODES_PER_CYCLE='${RL_COLLECT_EPISODES_PER_CYCLE:-}' RL_LR='${RL_LR:-}' RL_REPLAN_INTERVAL='${RL_REPLAN_INTERVAL:-}' RL_PROMPT_RESET='${RL_PROMPT_RESET:-0}' RL_RESUME='${RL_RESUME:-}' RL_KEYBOARD_Z_STEP_M='${RL_KEYBOARD_Z_STEP_M:-}' RL_KEYBOARD_PENALTY='${RL_KEYBOARD_PENALTY:-}' RL_DRY_RUN='${RL_DRY_RUN:-0}' CUDA_VISIBLE_DEVICES='${CUDA_VISIBLE_DEVICES:-0}' INFER_DEVICE='${INFER_DEVICE}' INFER_TASK_ID='${INFER_TASK_ID}' INFER_CHUNK_SIZE='${INFER_CHUNK_SIZE}' INFER_MAX_TIMESTEPS='${INFER_MAX_TIMESTEPS}' INFER_DISCRETE_DECODE='${INFER_DISCRETE_DECODE}' INFER_DISCRETE_TEMPERATURE='${INFER_DISCRETE_TEMPERATURE}' INFER_OBSERVATION_TIMEOUT_SEC='${INFER_OBSERVATION_TIMEOUT_SEC}' INFER_SLEEP_SEC='${INFER_SLEEP_SEC}' INFER_TARGET_DELTA_GAIN='${INFER_TARGET_DELTA_GAIN}' INFER_MAX_AMPLIFIED_STEP='${INFER_MAX_AMPLIFIED_STEP}' INFER_PRINT_ACTION='${INFER_PRINT_ACTION:-delta}' INFER_RECORD_CSV='${INFER_RECORD_CSV}' INFER_RESET_LIFT_Z_MIN_M='${INFER_RESET_LIFT_Z_MIN_M:-0.05}' INFER_RESET_LIFT_Z_MAX_M='${INFER_RESET_LIFT_Z_MAX_M:-0.10}' INFER_RESET_XY_JITTER_M='${INFER_RESET_XY_JITTER_M:-0.02}' INFER_RESET_SETTLE_SEC='${INFER_RESET_SETTLE_SEC:-2}' INFER_KEYBOARD_STOP='${INFER_KEYBOARD_STOP:-1}' ROS_DOMAIN_ID='${ROS_DOMAIN_ID}' USE_QPOS='${USE_QPOS}'; "
  fi
  local infer_auto_cmd="sleep 12; ${infer_hint}${infer_once_cmd}; echo '[infer] 已结束'; read"
  local infer_panel_cmd="export ACT_ROOT='${ACT_ROOT}'; export INFER_MODE='${MODE}'; export INFER_OBSERVATION_ZMQ='tcp://127.0.0.1:5554'; export INFER_TARGET_ZMQ='tcp://127.0.0.1:5555'; export INFER_PANEL_PORT='${INFER_PANEL_PORT}'; export INFER_CAPTURE_SCAN='0'; export INFER_OBSERVATION_TIMEOUT_SEC='${INFER_OBSERVATION_TIMEOUT_SEC}'; export INFER_DEVICE='${INFER_DEVICE}'; export INFER_MAX_TIMESTEPS='${INFER_MAX_TIMESTEPS}'; export INFER_TARGET_DELTA_GAIN='${INFER_TARGET_DELTA_GAIN}'; export INFER_CHUNK_SIZE='${INFER_CHUNK_SIZE}'; export INFER_SLEEP_SEC='${INFER_SLEEP_SEC}'; export INFER_DISCRETE_DECODE='${INFER_DISCRETE_DECODE}'; export INFER_DISCRETE_TEMPERATURE='${INFER_DISCRETE_TEMPERATURE}'; export USE_QPOS='${USE_QPOS}'; export BRUSH_CKPT_DIR='${BRUSH_CKPT_DIR}'; export BRUSH_TASK_NAME='${BRUSH_TASK_NAME}'; cd '${ROS2_WS_ROOT}'; python3 scripts/infer_control_panel.py; echo '[infer] 控制页已退出'; read"
  local infer_cmd="${aloha_env} export INFER_CHUNK_SIZE='${INFER_CHUNK_SIZE}'; export USE_QPOS='${USE_QPOS}'; ${rl_exports} cd '${ACT_ROOT}'; if [[ '${INFER_AUTO_START}' == '1' ]]; then ${infer_auto_cmd}; else ${infer_panel_cmd}; fi"
  ...
  echo "窗口: robot | pool | bridge | safety | infer | info"
  echo
  if [[ "${INFER_AUTO_START}" == "1" ]]; then
  ...
  echo "  ${SCRIPT_DIR}/run_infer_dualpool_paper.sh dry-run"
  echo
  echo "退出但不停止: Ctrl+B 然后 D"
  echo "停止全部: ${SCRIPT_DIR}/run_infer_dualpool_paper.sh kill"
  if [[ -t 1 ]]; then
    cmd_attach
  else
    echo "当前无交互终端，跳过 attach。进入会话: ${SCRIPT_DIR}/run_infer_dualpool_paper.sh attach"
  fi
}
